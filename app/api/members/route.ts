...
export async function POST(request: Request) {
  ...
  try {
    await env.DB!.batch([
      env.DB!.prepare("INSERT INTO members (user_id,username,email,display_name,role,status,created_at) VALUES (?,?,?,?,?,?,?)").bind(id, username, email, name, role, status, now),
  ...
  const member = await env.DB!.prepare("SELECT user_id AS id,username,email,display_name AS name,role,status,created_at AS createdAt FROM members WHERE user_id=?").bind(id).first();
