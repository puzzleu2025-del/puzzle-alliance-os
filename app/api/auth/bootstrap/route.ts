...
export async function POST(request: Request) {
  ...
  const alreadyEnabled = await env.DB.prepare("SELECT 1 FROM admin_credentials LIMIT 1").first();
  ...
  const result = await env.DB.prepare("INSERT INTO admin_credentials (user_id,password_hash,salt,updated_at) SELECT ?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM admin_credentials)")
