...
export default function SettingsPanel({preview,version,sync,audit,refresh,onLogout}:{preview:boolean;version:number;sync:string;audit:Audit[];refresh:()=>void;onLogout?:()=>void}) {
  ...
  return <><div className="page-heading"><div><h2>系統設定</h2><p className="muted">帳號安全、同步機制、角色權限與稽核紀錄</p></div></div><div className="settings-grid">
    ...
    ["總召","查看／協作","建立／確認","查看／匯出","查看","無"],
    ["幹部／組長","查看指派工作","查看／回覆","無","查看","無"],
    ["一般成員","查看與更新自己工作","查看／回覆","無","無","無"]
...
function AccountCard({preview,onLogout}:{preview:boolean;onLogout?:()=>void}) {
  ...
  const [account,setAccount]=useState<{username:string;displayName:string;hasPassword:boolean}|null>(null);
  ...
  useEffect(()=>{
    if(preview){
      ...
      setAccount({username,displayName:"",hasPassword:true});
      return;
    }
    let alive=true;
    fetch("/api/auth/account",{cache:"no-store"}).then(async response=>{const data=await response.json() as {username:string;displayName:string;hasPassword:boolean;error?:string};if(!response.ok)throw new Error(data.error||"無法讀取帳號");if(alive)setAccount(data);}).catch(error=>{if(alive)setMessage(error instanceof Error?error.message:"無法讀取帳號");});
    return ()=>{alive=false;};
  },[preview]);
  ...
  return <article className="card"><h3>帳號與密碼</h3><p>{account?.displayName||"目前登入帳號"}</p><small>{account?.username?`登入帳號：${account.username}`:"正在讀取帳號…"}</small>
