...
export type MemberRole = "admin" | "coordinator" | "leader" | "member";
...
function normalizeMember(value: unknown, keepPreviewPassword = false): Member | null {
  ...
  if (!id || !username || !name || !["admin", "coordinator", "leader", "member"].includes(role)
...
function Dialog({ title, close, children }: { title: string; close: () => void; children: ReactNode }) {
  ...
  return <dialog ref={ref} className="members-dialog" aria-labelledby="members-dialog-title" onCancel={(event) => { event.preventDefault(); close(); }} onClick={(event) => {
    if (event.target !== event.currentTarget) return;
    const rect = event.currentTarget.getBoundingClientRect();
    if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) close();
  }}><div className="members-dialog-head"><h2 id="members-dialog-title">{title}</h2><button type="button" aria-label="關閉" onClick={close}>×</button></div>{children}</dialog>;
}
...
export default function MembersPanel({ preview, currentUser, initialMembers }: Props) {
  ...
  const resetRequests = useMemo(() => members.filter((member) => member.resetPending), [members]);
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const member: Member = { id: crypto.randomUUID(), username, name, email: email || undefined, role, status, createdAt: new Date().toISOString() };
    setCreateBusy(true);
    setError("");
    try {
      if (preview) { const passwordHash = await previewPasswordHash(password); setMembers((rows) => [...rows, { ...member, password: passwordHash }]); }
      else await applyResponse(await fetch("/api/members", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ username, displayName: name, email: email || undefined, password, role, status }) }), member);
      setCreating(false);
    } catch (reason) {
  ...
  const patch = async (member: Member, change: Partial<Pick<Member, "role" | "status">>) => {
    if (busyId) return;
    if (change.role === "admin" && member.role !== "admin") {
      setError("系統目前只允許一位管理員，不能把其他成員升任系統管理員。");
      return;
    }
  ...
  return <section className="members-panel" aria-labelledby="members-title">
    <div className="page-heading"><div><h2 id="members-title">成員管理</h2><p className="muted">核可加入申請、設定角色並掌握目前使用狀態。</p></div><button className="primary" onClick={() => { setError(""); setCreating(true); }}>＋ 新增成員</button></div>
    ...
    <section className="members-pending" aria-labelledby="pending-title"><div className="members-section-head"><div><h3 id="pending-title">待核可申請</h3><p className="muted">確認身分與所需角色後再啟用。</p></div><span className="badge">{pending.length} 筆</span></div>{pending.length ? <div className="members-pending-list">{pending.map((member) => <article key={member.id}><div className="members-identity"><span className="members-avatar">{member.name.slice(0, 1).toUpperCase()}</span><div><b>{member.name}</b><small>@{member.username}</small>{member.email && <span>{member.email}</span>}<span>申請角色：{roleLabel(member.role)}</span></div></div><div className="members-actions"><select aria-label={`調整 ${member.name} 的角色`} value={member.role} disabled={busyId === member.id} onChange={(event) => void patch(member, { role: event.target.value as MemberRole })}>{roles.filter(([value]) => value !== "admin").map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><button className="primary" disabled={Boolean(busyId)} onClick={() => void patch(member, { status: "active" })}>核可</button><button className="danger" disabled={Boolean(busyId)} onClick={() => void patch(member, { status: "rejected" })}>拒絕</button></div></article>)}</div> : <p className="members-empty">目前沒有等待核可的申請。</p>}</section>

    {resetRequests.length > 0 && <section className="members-reset-requests" aria-labelledby="reset-title"><div className="members-section-head"><div><h3 id="reset-title">待重設密碼</h3><p>這些成員已提出忘記密碼申請；完成身分確認後再設定新密碼。</p></div><span className="members-reset-count">{resetRequests.length} 筆待處理</span></div><div className="members-pending-list">{resetRequests.map((member) => <article key={member.id}><div className="members-identity"><span className="members-avatar">{member.name.slice(0, 1).toUpperCase()}</span><div><b>{member.name}</b><small>@{member.username}</small>{member.email && <span>{member.email}</span>}</div></div><button className="primary" onClick={() => { setResetError(""); setResetTarget(member); }}>設定新密碼</button></article>)}</div></section>}

    <section className="members-directory" aria-labelledby="directory-title"><div className="members-section-head"><div><h3 id="directory-title">成員清單</h3><p className="muted">角色只決定工作空間內的操作範圍。</p></div><label>狀態<select value={filter} onChange={(event) => setFilter(event.target.value as MemberStatus | "all")}><option value="all">全部</option>{Object.entries(statusLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div>{loading ? <p className="members-empty" aria-live="polite">載入成員中…</p> : visible.length ? <div className="members-table-wrap"><table><thead><tr><th>成員</th><th>角色</th><th>狀態</th><th>加入時間</th><th>操作</th></tr></thead><tbody>{visible.map((member) => { const isCurrent = member.id === currentUser.id; return <tr key={member.id}><td><div className="members-identity"><span className="members-avatar">{member.name.slice(0, 1).toUpperCase()}</span><div><b>{member.name}{isCurrent ? "（你）" : ""}</b><small>@{member.username}</small>{member.email && <span>{member.email}</span>}{member.resetPending && <span className="members-reset-badge">待重設密碼</span>}</div></div></td><td><select aria-label={`調整 ${member.name} 的角色`} value={member.role} disabled={busyId === member.id || isCurrent} onChange={(event) => void patch(member, { role: event.target.value as MemberRole })}>{(member.role === "admin" ? roles : roles.filter(([value]) => value !== "admin")).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></td><td><span className={`members-status ${member.status}`}>{statusLabels[member.status]}</span></td><td>{new Date(member.createdAt).toLocaleDateString("zh-TW", { timeZone: "Asia/Taipei" })}</td><td>{member.status === "active" && !isCurrent ? <button disabled={Boolean(busyId)} onClick={() => void patch(member, { status: "disabled" })}>停用</button> : member.status === "disabled" ? <button disabled={Boolean(busyId)} onClick={() => void patch(member, { status: "active" })}>重新啟用</button> : member.status === "rejected" ? <button disabled={Boolean(busyId)} onClick={() => void patch(member, { status: "pending" })}>恢復申請</button> : null}</td></tr>; })}</tbody></table></div> : <p className="members-empty">沒有符合篩選條件的成員。</p>}</section>

    {creating && <Dialog title="新增成員" close={() => !createBusy && setCreating(false)}><form onSubmit={create}><fieldset disabled={createBusy}><label>登入帳號<input name="username" required minLength={4} maxLength={32} pattern="[A-Za-z0-9._-]+" autoComplete="username" autoFocus /><small>4–32 位，可使用英文字母、數字、句點、底線與連字號。</small></label><label>姓名<input name="name" required maxLength={100} autoComplete="name" /></label><label>Email（選填）<input name="email" type="email" maxLength={254} autoComplete="email" /></label><label>初始密碼<input name="password" type="password" required minLength={6} maxLength={256} autoComplete="new-password" /><small>至少 6 個字元；儲存後不會在介面中顯示。</small></label><label>角色<select name="role" defaultValue="member">{roles.filter(([value]) => value !== "admin").map(([value, label, description]) => <option value={value} key={value}>{label}｜{description}</option>)}</select></label><label>加入方式<select name="status" defaultValue="active"><option value="active">直接啟用</option><option value="pending">加入待核可清單</option></select></label></fieldset><p className="members-password-note">密碼只在新增時送交驗證服務；成員清單不會顯示或回填密碼。</p><div className="members-dialog-actions"><button type="button" disabled={createBusy} onClick={() => setCreating(false)}>取消</button><button className="primary" disabled={createBusy}>{createBusy ? "儲存中…" : "新增成員"}</button></div></form></Dialog>}
