...
export async function POST(request: Request) {
  ...
  try {
    await env.DB.batch([
      env.DB.prepare("INSERT INTO members (user_id,username,email,display_name,role,status,created_at) VALUES (?,?,?,?,?,'pending',?)").bind(userId, username, email, displayName, "member", now),
